
//----------------------------------------------------
// The following code was generated by CUP v0.11b beta 20140226
//----------------------------------------------------

/** CUP generated class containing symbol constants. */
public class sym {
  /* terminals */
  public static final int EKM = 18;
  public static final int DWORD = 19;
  public static final int PLUS = 22;
  public static final int ARR = 9;
  public static final int KMH = 17;
  public static final int CM = 7;
  public static final int STAR = 24;
  public static final int RO = 27;
  public static final int S = 6;
  public static final int DW = 21;
  public static final int COL = 8;
  public static final int EXPW = 14;
  public static final int RC = 28;
  public static final int TOW = 12;
  public static final int EURO = 20;
  public static final int EOF = 0;
  public static final int DIV = 25;
  public static final int EW = 15;
  public static final int TOK3 = 5;
  public static final int TOK2 = 4;
  public static final int DISCOUNT = 32;
  public static final int MINUS = 23;
  public static final int SEP = 2;
  public static final int TOK1 = 3;
  public static final int error = 1;
  public static final int TW = 13;
  public static final int UINT = 30;
  public static final int QSTRING = 29;
  public static final int PC = 16;
  public static final int KMW = 10;
  public static final int REAL = 31;
  public static final int CW = 11;
  public static final int UMINUS = 26;
  public static final String[] terminalNames = new String[] {
  "EOF",
  "error",
  "SEP",
  "TOK1",
  "TOK2",
  "TOK3",
  "S",
  "CM",
  "COL",
  "ARR",
  "KMW",
  "CW",
  "TOW",
  "TW",
  "EXPW",
  "EW",
  "PC",
  "KMH",
  "EKM",
  "DWORD",
  "EURO",
  "DW",
  "PLUS",
  "MINUS",
  "STAR",
  "DIV",
  "UMINUS",
  "RO",
  "RC",
  "QSTRING",
  "UINT",
  "REAL",
  "DISCOUNT"
  };
public String[] TT;
	public sym(){
		TT = new String[100];
		TT[18]=new String("EKM");
		TT[19]=new String("DWORD");
		TT[22]=new String("PLUS");
		TT[9]=new String("ARR");
		TT[17]=new String("KMH");
		TT[7]=new String("CM");
		TT[24]=new String("STAR");
		TT[27]=new String("RO");
		TT[6]=new String("S");
		TT[21]=new String("DW");
		TT[8]=new String("COL");
		TT[14]=new String("EXPW");
		TT[28]=new String("RC");
		TT[12]=new String("TOW");
		TT[20]=new String("EURO");
		TT[0]=new String("EOF");
		TT[25]=new String("DIV");
		TT[15]=new String("EW");
		TT[5]=new String("TOK3");
		TT[4]=new String("TOK2");
		TT[32]=new String("DISCOUNT");
		TT[23]=new String("MINUS");
		TT[2]=new String("SEP");
		TT[3]=new String("TOK1");
		TT[1]=new String("error");
		TT[13]=new String("TW");
		TT[30]=new String("UINT");
		TT[29]=new String("QSTRING");
		TT[16]=new String("PC");
		TT[10]=new String("KMW");
		TT[31]=new String("REAL");
		TT[11]=new String("CW");
		TT[26]=new String("UMINUS");
	}
public String getTT(int i){return TT[i];}
}

