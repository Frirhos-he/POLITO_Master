...
email =	([a-zA-Z0-9_\.])+"@"([a-zA-Z])+"."(it|org|com)
...
token1 = \?{word}{oct}({shortw})?

word = [A-Z]{6}([A-Z]{2})*
oct = -( (12[0-7]) | (1[01][0-7]) | ([0-7]{1,2}) ) |
	  ([0-7]{1,2})|
	  ([12][0-7]{2})|
	  (3[01][0-7]) | (32[0-3])
shortw = (xx|yy|zz){4}((xx|yy|zz))*

token2 = {email} (([\!\/]{email}) | ([\!\/]{email}){11} | ([\!\/]{email}){14})
...
"T" 			{if(test)System.out.print(yytext());return sym(sym.T);} 
"F" 			{if(test)System.out.print(yytext());return sym(sym.F);} 
...
"CMP" 			{if(test)System.out.print(yytext());return sym(sym.CMPW);} 
"WITH" 			{if(test)System.out.print(yytext());return sym(sym.WW);} 
"print" 			{if(test)System.out.print(yytext());return sym(sym.PW);} 
"fz_and" 			{if(test)System.out.print(yytext());return sym(sym.FW);} 
